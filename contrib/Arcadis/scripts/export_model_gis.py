...
def export_model_gis(mdu_path,nc_path,output_path):
    ...
    network["1d_meshnodes"] = network["1d_meshnodes"].rename(columns={"id": "meshnodeid"})
    network["1d_meshnodes"] = network["1d_meshnodes"].rename(columns={"branch": "branchnr"})
    network["1d_meshnodes"] = pd.merge(network["1d_meshnodes"], branche_ids , on="branchnr")         
    ...
    gdfs_results = read_locations(
        mdu_path,
        ["cross_sections_locations","structures","boundaries","laterals"])
    gdfs_results["cross_sections_locations"] = gdfs_results["cross_sections_locations"][['id', 'branchid', 'chainage', 'shift', 'definitionid','geometry']]
    gdfs_results["cross_sections_locations"] = gdfs_results["cross_sections_locations"].rename(columns={"definitionid": "def_id", "locationtype": "type"})
    gdfs_results["structures"] = gdfs_results["structures"][['id', 'name', 'type', 'branchid', 'chainage','geometry']]
    gdfs_results["laterals"] = gdfs_results["laterals"][['id', 'nodeid', 'branchid', 'chainage','geometry']]
...
if __name__ == "__main__":
    ...
    nc_path = Path(r"C:\Users\devop\Documents\Scripts\Hydrolib\HYDROLIB\contrib\Arcadis\scripts\exampledata\Dellen\Model_cleaned\dflowfm\dellen_net.nc")
