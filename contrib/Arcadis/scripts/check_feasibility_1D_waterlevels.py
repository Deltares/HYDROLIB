...
def check_feasibility_1D_waterlevels(mdu_path, nc_path, output_path):
    ...
    meshnodes = pd.merge(
        meshnodes, branches_reduced, on="branchnr"
    )  # The dataframe of the meshnodes do include a branch number, but not a branch id
    ...
    total_nodes_per_branch = pd.merge(
        total_nodes_per_branch, branches_reduced, on="branchnr"
    )
    ...
    for loc_id in loc_ids:
        ...
        if (
            ...
            crslocs.loc[
                index_loc, "method"
            ] = "cannot find embankment height because it is not a yz-profile"
        else:
            ...
            if len(meshnodes_selection) == 0:
                ...
                if len(meshnodes_selection) == 1:
                    crslocs.loc[index_loc, "max_waterlevel"] = meshnodes_selection[
                        "max_waterlevel"
                    ].iloc[0]
                    crslocs.loc[
                        index_loc, "method"
                    ...
                    ):  # there is a structure on the same branch, now determine if one lies in between the cross section and selected mesh nodes
                    ...
                    if (
                        ...
                        if (
                            ...
                            slope = (
                                meshnodes_selection.loc[1, "max_waterlevel"]
                                - meshnodes_selection.loc[0, "max_waterlevel"]
                            ) / (
                                meshnodes_selection.loc[1, "offset"]
                                - meshnodes_selection.loc[0, "offset"]
                            )
                            crslocs.loc[index_loc, "max_waterlevel"] = (
                                slope
...
if __name__ == "__main__":
    ...
    nc_path = Path(
        r"C:\Users\devop\Documents\Scripts\Hydrolib\HYDROLIB\contrib\Arcadis\scripts\exampledata\Zwolle-Minimodel_clean\1D2D-DIMR\dflowfm\output\FlowFM_map.nc"
    )
