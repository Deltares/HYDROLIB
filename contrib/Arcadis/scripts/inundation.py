...
def inun_dhydro(map_path,output_folder,type="level",dtm_path="",sdate="",edate="",filter=False):
    interpol = 0.5
    ...
    if type == "level":
        gdf2_buf = gpd.GeoDataFrame(gdf2.copy(),geometry=gdf2.buffer(gdf2.area**0.5 * interpol))
    ...
    with rasterio.open(os.path.join(output_folder, filename), 'w+', **meta) as out:
        ...
        values = features.rasterize(shapes=shapes, fill=0, out=out_arr, transform=out.transform)
    ...
    if type == "level":
        ...
        if filter == True:
            ...
            mask = None
            results = ({'properties': {'raster_val': v}, 'geometry': s} for i, (s, v) in enumerate(rasterio.features.shapes(dummy, mask=mask, transform=src.transform)))
            geoms = list(results)            
            ...
            inun_gdf_buf = gpd.GeoDataFrame(inun_gdf,geometry=inun_gdf.buffer(resx)) # todo: beter nadenken over buffer
            inun_gdf_buf["dum"] = 1
            inun_gdf_dis = inun_gdf_buf.dissolve(by="dum").explode()
            inun_gdf_sel = inun_gdf_dis[inun_gdf_dis.area > cellsize**2 * 2].reset_index() # todo: beter nadenken over oppervlak
            shapes = ((geom,value) for geom, value in zip(inun_gdf_sel.geometry, inun_gdf_sel["dum"]))
            nan_array = np.empty((len(out_arr),len(out_arr[0]))).astype('float32')
            nan_array[:] = np.nan
            filter = rasterio.features.rasterize(shapes=shapes, fill=0,out=nan_array, transform=out.transform)
...
if __name__ == '__main__':
    ...
    inun_dhydro(input_path,output_folder,type=type,dtm_path=dtm_path,sdate="",edate="")