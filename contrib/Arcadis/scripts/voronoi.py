...
def voronoi_extra(gdf_points, gdf_areas, group_id=''): 
    ...
    gdf_points = gdf_points[~gdf_points.index.duplicated(keep='first')] # verwijder dubbele punten in Sobek
    ...
    for index, row in gdf_areas.iterrows():
        ...
        gdf_points_clip = gpd.sjoin(gdf_points, gpd.GeoDataFrame(geometry=[area_geom],crs=gdf_areas.crs), predicate = 'within')
        ...
        if len(gdf_points_clip) == 0: # no points, skip
            ...
            if len(gdf_points_clip) < 4: # not enough points
                ...
                gdf_points_dummy['geometry'] = gdf_points_dummy.translate(xoff=0.001, yoff=0.001)
                gdf_points_extra_clip = gpd.GeoDataFrame(pd.concat([gdf_points_clip, gdf_points_dummy], ignore_index=True),crs=gdf_points.crs)
            ...
            if pkg_resources.get_distribution("geovoronoi").version < "0.4.0":
                ...
                xmin, ymin, xmax, ymax = area_geom.buffer(area_geom.area**0.5*10).envelope.bounds
                ...
                gdf_points_extra_clip = gdf_points_extra_clip.append([point1,point2,point3,point4])               
            ...
            try:
                if pkg_resources.get_distribution("geovoronoi").version >= "0.4.0":
                    ...
                    vr_area, vr_points, vr_links = voronoi_regions_from_coords(coords, area_geom)
            except:
                # if voronoi fails, try by making bigger voronois and clip to area
                try:
                    if pkg_resources.get_distribution("geovoronoi").version >= "0.4.0":
                        vr_area, vr_points = voronoi_regions_from_coords(coords, tot_geom)
                    else:
                        vr_area, vr_points, vr_links = voronoi_regions_from_coords(coords, tot_geom)
                except:
            ...
            if pkg_resources.get_distribution("geovoronoi").version >= "0.4.0":
                ...
                voronoi = gpd.GeoDataFrame([gdf_points_extra_clip.iloc[value] for key, value in vr_points.items()],geometry=[value for key, value in vr_area.items()],crs=gdf_points.crs)
            else:
                voronoi = gpd.GeoDataFrame([gdf_points_extra_clip.iloc[id[0]] for id in vr_links],geometry=vr_area,crs=gdf_points.crs)
    ...
    gdf_result = gdf_result.dissolve(by='indx', aggfunc = 'first') # dissolve to prevent dupplicates due to not enough points
