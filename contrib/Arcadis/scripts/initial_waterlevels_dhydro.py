...
from hydrolib.core.io.net.models import Network, Link1d2d, Mesh1d
from read_dhydro import net_nc2gdf

## AHT specific funcions

def aht_initial_dhydro(net_nc_path,areas_path,value_field,value_type,global_value,output_path):
    """
    ...
    with open(output_location, "w") as f:
        ...
        for initial in initials:
            ...
            if len(set(initial[2])) == 1:
                f.write("    values                = " + "{:8.3f}".format(initial[2][0]) + "\n")
            else:
                f.write("    numLocations          = " + str(len(initial[2])) + "\n")
                f.write("    chainage              = " + " ".join(["{:8.3f}".format(x) for x in initial[1]]) + "\n")
                f.write("    values                = " + " ".join(["{:8.3f}".format(x) for x in initial[2]]) + "\n")
