import numpy as np
from shapely.geometry import MultiPolygon,LineString, MultiPoint, Point,Polygon
...
def write_pli(gdf,output,id="unique_id",values=[],write_z=True):
        ...
        for i in range_line:
            if geom_type == {"Point"}:
                ...
                for j in range_coords: # loop over vertex in line segment
                    ...
                    if (i == 0 and j == 0) or (i == range_line[-1] and j == range_coords[-1]) or (i in range_line[1:-1] or j in range_coords[1:-1]):
                    ...
                    # end of linepart
                        ...
                        if write_z and check_z.iloc[i]:
                            values_list = [line.iloc[i].geometry.coords[j][2],line.iloc[i+1].geometry.coords[0][2]]
                            values_list = [value_list for value_list in values_list if value_list>-9999]
                            item.append(-9999.0 if len(values_list) == 0 else sum(values_list)/len(values_list))    
                        for value in values:
    ...
    with open(output, 'w') as file:
        for ID in pli_dict:
            ...
            for part in line:
                file_string = "{:11.3f}".format(part[0]) + " " + "{:11.3f}".format(part[1]) 
                for i in part[2:]:
