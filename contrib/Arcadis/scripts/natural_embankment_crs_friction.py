...
from clean_dhydro import (
    clean_crsdefs,
    clean_friction_files,
    remove_double_friction_definitions,
...
def natural_embankment_crs_friction(
    ...
    loc_ids = crslocs.loc[
        crslocs.intersects(areas_selection.dissolve().geometry.iloc[0]), "id"
    ...
    crslocs, crsdefs, dict_frictions = remove_double_friction_definitions(
        crslocs, crsdefs, dict_frictions
    )
    ...
    crslocs_selection = gpd.sjoin(
        crslocs_selection, areas_selection, predicate="within"
    )
    if column_waterlevel in crslocs_selection.columns:
        crslocs_selection = crslocs_selection.rename(
            columns={column_waterlevel: "waterlevel"}
        )
    else:
    ...
    for loc_id in loc_ids:
        ...
        index_loc = crslocs_selection.loc[
            crslocs_selection["id"] == loc_id
        ].index.to_list()[0]
        ...
        if crsdefs.loc[index_def, "type"] == "yz":
            ...
            crsdefs.loc[index_def, "waterlevel"] = crslocs_selection.loc[
                index_loc, "waterlevel"
            ].copy()
            ...
            while found == 0 and i < len(y) - 2:
                ...
                if y_step == crsdefs.loc[index_def, "waterlevel"]:
                    ...
                    z_step > crsdefs.loc[index_def, "waterlevel"]
                    and z_step_further < crsdefs.loc[index_def, "waterlevel"]
                ):
                    if y_step != y_step_further:  # if there is a vertical embankment
                        crsdefs.loc[index_def, "left_y_waterlevel"] = (
                            abs(
                                (z_step - crsdefs.loc[index_def, "waterlevel"])
                                / (
                                    (z_step_further - z_step)
                                    / (y_step_further - y_step)
                                )
                            )
                            + y_step
                        )
            ...
            while found == 0 and i > 0:
                ...
                if y_step == crsdefs.loc[index_def, "waterlevel"]:
                    ...
                    if y_step != y_step_further:  # if there is a vertical embankment
                        crsdefs.loc[index_def, "right_y_waterlevel"] = y_step - abs(
                            (z_step - crsdefs.loc[index_def, "waterlevel"])
                            / ((z_step_further - z_step) / (y_step_further - y_step))
                        )
                    else:
                        crsdefs.loc[index_def, "right_y_waterlevel"] = y_step
                    found = 1
            ...
            if (crsdefs.loc[index_def, "left_y_waterlevel"] == y[0]) and (
                crsdefs.loc[index_def, "right_y_waterlevel"] == y[-1]
            ):
                print(
                    "Warning, cross section def "
                    + def_id
                    + " lies entirely below/above waterlevel"
                )
            if (
                width_embak + crsdefs.loc[index_def, "left_y_waterlevel"]
                > crsdefs.loc[index_def, "right_y_waterlevel"]
            ):
                print(
                    "Natural embankment for cross section def " + str(def_id),
                    "is wider than wet area, therefore embankment is set equal to wet area",
                )
                crsdefs.loc[index_def, "sectioncount"] = 1
                ...
                if side == "left":
                    ...
                    crsdefs.at[index_def, "frictionpositions"] = [
                        y[0],
                        round(
                            width_embak + crsdefs.loc[index_def, "left_y_waterlevel"], 3
                        ),
                        y[-1],
                    ]
                    crsdefs.at[index_def, "frictionids"] = [
                        "Naturalembankment",
                        "Flowsection",
                    ]
                else:
                    ...
                    crsdefs.at[index_def, "frictionpositions"] = [
                        y[0],
                        round(
                            crsdefs.loc[index_def, "right_y_waterlevel"] - width_embak,
                            3,
                        ),
                        y[-1],
                    ]
            ...
            for check_file in friction_files:
                if len(dict_frictions[check_file]) > 0:
                    branches_check_file = dict_frictions[check_file][
                        "branchid"
                    ].to_list()
                    if branch_id in branches_check_file:
                        i = dict_frictions[check_file].index[
                            dict_frictions[check_file]["branchid"] == branch_id
                        ][0]
                        dict_frictions[check_file] = dict_frictions[check_file].drop(
                            labels=i, axis=0
                        )

    ## WRITE CROSS SECTION DATA
    crsdefs = crsdefs.drop(
        columns=["waterlevel", "left_y_waterlevel", "right_y_waterlevel", "method"]
    )
    ...
    if os.path.isfile(os.path.join(output_path, "roughness-Channels.ini")):
        ...
        os.path.join(output_path, "roughness", "roughness-Channels.ini")
    ):
    ...
    if type(friction_type) == int:
        if friction_type == 0:
            ...
            raise Exception(
                "Friction type was entered as an integer, but wrong integer was given"
            )
    if friction_type not in [
        "Chezy",
        "Manning",
        "WallLawNikuradse",
        "WhiteColebrook",
...
if __name__ == "__main__":
    ...
    shape_path = Path(
        r"C:\Users\devop\Documents\Scripts\Hydrolib\HYDROLIB\contrib\Arcadis\scripts\exampledata\Dellen\GIS\nvo_test.shp"
    )
    ...
    natural_embankment_crs_friction(
        mdu_path,
        shape_path,
        column_waterlevel,
        width_embak,
