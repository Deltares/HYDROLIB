...
from hydrolib.core.io.mdu.models import FrictionModel
...
def read_frictionfiles(fm):
    ...
    for i in range(len(fm.geometry.frictfile)):
        ...
        dfsglob[
            str(str(fm.geometry.frictfile[i].global_[0].frictionid))
        ] = dfglob
...
if __name__ == "__main__":
    ...
    output_location = r"C:\scripts\AHT_scriptjes\Hydrolib\Dhydro_changefrict\roughness-Section000.ini"
    input_mdu = Path(r"C:\scripts\AHT_scriptjes\Hydrolib\Dhydro_changefrict\data\Zwolle-Minimodel\1D2D-DIMR\dflowfm\flowFM.mdu")
    ...
    intersect = gpd.sjoin_nearest(
        branches, gdf_frict, max_distance=max_distance
    )  
    # TODO: Probleem Arjon
        ...
        if not dict_chann[file].empty:
            for id in dict_chann[file].branchid:
                if id in intersect['id'].values:
                    ...
                    dict_chann[file].at[dict_chann[file].loc[dict_chann[file].branchid == id].index.to_list()[0],'frictionvalues'] = list(intersect.loc[intersect["id"] == id, "fricval"])
    for file in dict_chann:
