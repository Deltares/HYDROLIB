...
def change_friction_shape(mdu_path, shape_path, netnc_path, output_path):
    ...
    for key in dict_frictions:
        if not dict_frictions[key].empty:
            ...
            in_model = intersect.assign(result=intersect['id'].isin(dict_frictions[key].branchid))
            ...
            merge.loc[merge.result == True,'frictionvalues'] = merge.loc[merge.result == True,'fricval']
            ...
            merge['frictionvalues'] = merge['frictionvalues'].apply(lambda x: [x] if isinstance(x, float) else x)
            merge['chainage'] = merge['chainage'].apply(lambda x: [x] if isinstance(x, int) else x)
            merge.drop(columns = in_model.columns.difference(dict_frictions[key].columns), inplace = True)
