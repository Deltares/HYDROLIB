...
from read_dhydro import net_nc2gdf, pli2gdf, chainage2gdf, branch_gui2df, read_nc_data, map_nc2gdf
...
def statistics_dhydro(input_path, par, sdate="",edate="",output_path="/tmp/dhydro.shp",stat=""):
    """
   ...
   """
    ...
    if extension not in [".csv",".xlsx",".shp",".tif"]:
        ...
        print ("Onbekende extensie " + str(extension) + "\n" + "Geen output file gegenereerd")
...
# =============================================================================
    ...
    # create geometry
    network_type = "2d_faces" if "mesh2d" in ds[par].mesh.lower() else "1d_meshnodes" # todo line and structure info    gdfs = net_nc2gdf(input_path,results=[network_type])
    gdfs = net_nc2gdf(input_path,results=[network_type])
    gdf = gdfs[network_type.lower() if network_type not in list(gdfs.keys()) else network_type]
    ...
    if extension in [".csv",".xlsx",".shp",".tif"]:    
        if extension == ".xlsx":
            if isinstance(gdf, pd.DataFrame):
                gdf.to_excel(output_path, float_format="%.3f", sheet_name=str(par),index=True) 
            else:
                df_stat.to_excel(output_path, float_format="%.3f", sheet_name=str(par),index=True) 
        elif extension == ".shp":
            if isinstance(gdf, pd.DataFrame):
            ...
            gdf_area = gpd.GeoDataFrame(index=[0], crs="EPSG:28992", geometry=[geom_area])
            meta = "fout!!" '<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'
            with rasterio.open(output_path, 'w+', **meta) as out:
                out_arr = out.read(1)
                shapes = ((geom,value) for geom, value in zip(gdf.geometry, gdf[stat]))
                level = rasterio.features.rasterize(shapes=shapes, fill=0, out=out_arr, transform=out.transform)
...
if __name__ == "__main__":
    ...
    par = 'mesh1d_q1'
    test = statistics_dhydro(input_path, par, sdate="",edate="",output_path="",stat="")
    print ('test')