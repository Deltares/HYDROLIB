...
def create_branches(network_nc, output_folder=False):
    ...
    df_network = pd.concat(
        [pd.Series(ds[geom_x].values), pd.Series(ds[geom_y].values)], axis=1
    )
    df_network.columns = [geom_x, geom_y]
    gdf_network = gpd.GeoDataFrame(
        df_network, geometry=gpd.points_from_xy(df_network[geom_x], df_network[geom_y])
    )
    ...
    for j in range(len(df_branches)):
        ...
        linestring = LineString(
            gdf_network.iloc[df_branches["start_node"][j] : df_branches["end_node"][j]][
                "geometry"
            ].values
        )
...
def create_crosssections(
    branches: gpd.GeoDataFrame, crossloc_ini: Path, output_folder=False
):
    """
...
def select_crosssection_locations(crosssection_locations, shapefile_path):
    ...
    selected_crosssection_locations.rename(
        columns={"cross_loc_geom": "geometry"}, inplace=True
    )
