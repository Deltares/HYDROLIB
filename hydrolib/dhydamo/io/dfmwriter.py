...
class DFLowFMModelWriter:
    ...
    def crosssection_locations_to_dhydro(self):
        css = [
            CrossSection(**cloc)
            for cloc in self.hydamo.crosssections.crosssection_loc.to_dict("records")
        ]
    ...
    def crosssection_definitions_to_dhydro(self):
        cs_circle = self.hydamo.crosssections.crosssection_def[
            self.hydamo.crosssections.crosssection_def.type == "circle"
        ]
        cs = [CircleCrsDef(**cs) for cs in cs_circle.to_dict("records")]
        [[setattr(c.comments, field[0], "") for field in c.comments] for c in cs]
        self.crossdefs += cs
        cs_yz = self.hydamo.crosssections.crosssection_def[
            self.hydamo.crosssections.crosssection_def.type == "yz"
        ]
        cs = [YZCrsDef(**cs) for cs in cs_yz.to_dict("records")]
        [[setattr(c.comments, field[0], "") for field in c.comments] for c in cs]
        self.crossdefs += cs
        cs_rect = self.hydamo.crosssections.crosssection_def[
            self.hydamo.crosssections.crosssection_def.type == "rectangle"
        ]
    ...
    def boundaries_to_dhydro(self, forcingmodel):
        for bound in self.hydamo.external_forcings.boundary_nodes.itertuples():
            bnd_ext = Boundary(
                nodeid=bound.nodeid, quantity=bound.quantity, forcingfile=forcingmodel
            )
    ...
    def storagenodes_to_dhydro(self):
        stornodes = [
            ...
            for stornode in self.hydamo.storagenodes.storagenodes.to_dict("records")
        ]
