...
class UnpavedIO:
    ...
    def unpaved_from_input(
        ...
        for num, cat in enumerate(catchments.itertuples()):
            ...
            if isinstance(surface_storage, float):
                ...
                unpaved_drr.at[cat.code, "surface_storage"] = f'{sstores[num]["mean"]:.3f}'
            if isinstance(infiltration_capacity, float):
                unpaved_drr.at[cat.code, "infiltration_capacity"] = f"{infiltration_capacity:.3f}"
            else:
                unpaved_drr.at[cat.code, "infiltration_capacity"] = f'{infcaps[num]["mean"]:.3f}'
        ...
        [self.unpaved.add_unpaved(**unpaved) for unpaved in unpaved_drr.to_dict('records')]
    ...
    def ernst_from_input(
        ...
        ernst_drr.set_data( pd.DataFrame(np.zeros((len(catchments),5)), 
                                       columns=['id','cvo','lv','cvi','cvs'], dtype="str"), index_col='id')
...
class PavedIO:
    ...
    def paved_from_input(
        ...
        if isinstance( street_storage, str):                
            strs_rast, strs_affine = self.paved.drrmodel.read_raster(street_storage, static=True)
        ...
        if isinstance( sewer_storage, str):                
            sews_rast, sews_affine = self.paved.drrmodel.read_raster(sewer_storage, static=True)
        ...
        if isinstance(pump_capacity, str):     
            pump_rast, pump_affine = self.paved.drrmodel.read_raster(pump_capacity, static=True)               
        ...
        if sewer_areas is not None:        
            ...
            paved_drr.set_data( pd.DataFrame(np.zeros((len(catchments)+len(overflows),10)), 
                                columns=['id','area','surface_level', 'street_storage', 'sewer_storage', 'pump_capacity','meteo_area','px', 'py', 'boundary_node'], dtype="str"), index_col='id')
            ...
            for isew, sew in enumerate(sewer_areas.itertuples()):            
                ...
                for ov in ovf.itertuples():                
                    ...
                    if isinstance(street_storage, float):    
                        paved_drr.at[ov.code, 'street_storage'] = f'{street_storage:.2f}'
                    else:
                        paved_drr.at[ov.code, 'street_storage'] = f'{str_stors_sa[isew]["mean"]:.2f}'                   
                    if isinstance(sewer_storage, float):    
                        paved_drr.at[ov.code, 'sewer_storage'] = f'{sewer_storage:.2f}'
                    else:
                        paved_drr.at[ov.code, 'sewer_storage'] = f'{sew_stors_sa[isew]["mean"]:.2f}'            
                    if isinstance(pump_capacity, float):    
                        paved_drr.at[ov.code, 'pump_capacity'] = f'{pump_capacity}'
                    else:
                        paved_drr.at[ov.code, 'pump_capacity'] = f'{pump_caps_sa[isew]["mean"]:.2f}'    
        ...
        for num, cat in enumerate(catchments.itertuples()):                    
            ...
            if isinstance(street_storage, float):    
                ...
                paved_drr.at[cat.code, 'street_storage'] = f'{str_stors[num]["mean"]:.2f}'                   
            if isinstance(sewer_storage, float):    
                paved_drr.at[cat.code, 'sewer_storage'] = f'{sewer_storage:.2f}'
            else:
                paved_drr.at[cat.code, 'sewer_storage'] = f'{sew_stors[num]["mean"]:.2f}'            
            if isinstance(pump_capacity, float):    
                paved_drr.at[cat.code, 'pump_capacity'] = f'{pump_capacity}'
            else:
                paved_drr.at[cat.code, 'pump_capacity'] = f'{pump_caps[num]["mean"]:.2f}'    
...
class GreenhouseIO:
    ...
    def greenhouse_from_input(
        ...
        if isinstance(roof_storage, str):                
            rast, affine = self.greenhouse.drrmodel.read_raster(roof_storage, static=True)
        ...
        gh_drr.set_data( pd.DataFrame(np.zeros((len(catchments),8)), 
                                        columns=['id','area','surface_level', 'roof_storage', 'meteo_area','px', 'py', 'boundary_node'], dtype="str"), index_col='id')
...
class OpenwaterIO:
    ...
    def openwater_from_input(
        self, catchments:ExtendedGeoDataFrame, landuse:str, meteo_areas:ExtendedGeoDataFrame, zonalstats_alltouched:bool=None
    )->None:
        ...
        ow_drr.set_data( pd.DataFrame(np.zeros((len(catchments),6)), 
                                        columns=['id','area','meteo_area','px', 'py', 'boundary_node'], dtype="str"), index_col='id')
...
class ExternalForcingsIO:
    ...
    @validate_arguments(config=dict(arbitrary_types_allowed=True))
    def seepage_from_input(self, catchments:ExtendedGeoDataFrame, seepage_folder:str) -> None:
        ...
        for ifile, file in tqdm(enumerate(file_list),total=len(file_list),desc='Reading seepage files'):
            array, affine, time = self.external_forcings.drrmodel.read_raster(os.path.join(seepage_folder, file))
            times.append(time)           
    ...
    @validate_arguments(config=dict(arbitrary_types_allowed=True))
    def precip_from_input(self, areas:ExtendedGeoDataFrame  , precip_folder:str=None, precip_file:str=None) -> None:
        ...
        if precip_file is None:            
            ...
            for ifile, file in tqdm(enumerate(file_list),total=len(file_list),desc='Reading precipitation files'):
                array, affine, time = self.external_forcings.drrmodel.read_raster(os.path.join(precip_folder, file))
                times.append(time)                   
    ...
    @validate_arguments(config=dict(arbitrary_types_allowed=True))
    def evap_from_input(self, areas:ExtendedGeoDataFrame, evap_folder:str=None, evap_file:str=None) -> None:
        ...
        if evap_file is None:        
            ...
            for ifile, file in tqdm(enumerate(file_list),total=len(file_list),desc='Reading evaporation files'):
                array, affine, time = self.external_forcings.drrmodel.read_raster(os.path.join(evap_folder, file))
                times.append(time)                  
                stats = zonal_stats(areas, array, affine=affine, stats="mean",all_touched=True)
                arr[ifile,:] = [s['mean'] for s in stats]       
            result = pd.DataFrame(arr, columns=['ms_'+str(area) for area in areas.code])
    ...
    @validate_arguments(config=dict(arbitrary_types_allowed=True))
    def boundary_from_input(self, boundary_nodes:ExtendedGeoDataFrame, catchments:ExtendedGeoDataFrame, drrmodel, overflows:ExtendedGeoDataFrame=None) -> None:
        ...
        bnd_drr.set_data( pd.DataFrame(np.zeros((numlats,3)), 
                                        columns=['id', 'px', 'py'], dtype="str"), index_col='id')                                        
        if overflows is not None:
        ...
        [self.external_forcings.add_boundary_node(**bnd) for bnd in bnd_drr.to_dict('records')]
