...
from delft3dfmpy import OSM, HyDAMO, DFlowFMModel, Rectangular, DFlowFMWriter, DFlowRRWriter
...
def read_gpd(gpdfile, id_col: str = None, rename_map: dict = None, required_columns: list = None,
             required_dtypes: list = None, clip_geom: gpd.GeoDataFrame = None, clip_predicate:str = "contains"):
    ...
    if required_columns is None and required_dtypes is None:
        ...
        if required_columns is not None and required_dtypes is None:
            ...
            try:
                logging.debug(f'Ensuring data types with required_dtypes: {required_columns}')
                data = gpd.GeoDataFrame(geometry=_data['geometry'])
                for c, dc in zip(required_columns, required_dtypes):
                    _data[c] = _data[c].replace({'NULL': None, 'None': None, 'none': None
                                                 })
                    if dc == 'bool':
                        _data[c] = _data[c].replace({'True': True, 'True': True, 'true': True,
...
def read_raster(raster_fn:str, id_col: str = None, clip_geom: gpd.GeoDataFrame = None, nodata = -999):
...
def export_to_fews(exportpath, pilotname, pilotcrs, dfmmodel, drrmodel, staticgeoms, guipath = None, logger = logging):
    ...
    rr_writer.d3b_parameters.update({'OutputOptions': {'OutputRRUnpaved': 0,
                                                       'OutputRRNWRW': 0},
                                     'Options': {'GenerateNetCdfOutput': -1,
                                                 'GenerateHisOutput': 0,
                                                 'MeteoNetCdfInput': -1,
                                                 ...
                                                 'RestartOut': 0}})  # FIXME BMA: need to change to -1 when starting to use restart
    ...
    zout = zipfile.ZipFile(zname, "w", zipfile.ZIP_DEFLATED) # <--- this is the change you need to make
    ...
    fews_csv = pd.DataFrame(
        [dfmmodel.network.mesh1d.description1d['mesh1d_node_ids'], dfmmodel.network.mesh1d.get_values('nodex'),
         dfmmodel.network.mesh1d.get_values('nodey')]).T
    ...
    if guipath is not None:
        dimrpath = guipath.joinpath( r"plugins\DeltaShell.Dimr\kernels\x64\dimr\scripts\run_dimr.bat")
...
def setup_datamodel(data: gpd.GeoDataFrame, datamodel_type: str, datamodel_map: dict = None) -> ExtendedGeoDataFrame:
    ...
    if datamodel_type == 'branches':
        datamodel_map = {
            "id": "BRANCH_ID",
            "name": "BRANCH_ID",
            "branchType": {'branchType == "Channel"': 0, 'branchType == "SewerConnection"': 1,
                           'branchType == "Pipe"': 2},
            'material': {'MATERIAL == "concrete"': 1, 'MATERIAL == "clay"': 0,
                         'MATERIAL == "CastIron"': 2, 'MATERIAL == "StoneWare"': 3,
                         'MATERIAL == "Hdpe"': 4, 'MATERIAL == "Masonry"': 5,
                         'MATERIAL == "SheetMetal"': 6, 'MATERIAL == "Polyester"': 7,
                         'MATERIAL == "Polyvinylchlorid"': 8, 'MATERIAL == "Steel"': 9},
...
def setup_pilot(pilot_fn: str = None,
...
# def setup_branches
def setup_branches(branches: gpd.GeoDataFrame = None,
    ...
    if branches is None and branches_fn is not None:
        ...
        branches = read_gpd(branches_fn, id_col, rename_map, required_columns, required_dtypes, clip_geom=pilot.buffer(10), clip_predicate="intersects") # use clip_predicate="intersects" for reduced domain - more branches with boundary nodes
    ...
    if branches is not None:
        # preprocess
        branches = _preprocess_branches(branches, branches_ini, snap_offset = snap_offset, id_col = id_col, logger = logger)
    ...
    if sum(branches.geometry.length <= 0) == 0:
        ...
        logger.error(f'Branches {branches.index[branches.geometry.length <= 0]} have length of 0 meter. ' +
...
def _preprocess_branches(branches:gpd.GeoDataFrame,
    ...
    for branch_index, branch in branches.iterrows():
        if branch.geometry.type != 'LineString':
            branches.at[branch_index, 'geometry'] = LineString([p for l in branch.geometry for p in l.coords])
            n += 1
    ...
    if id_col in ['None', 'NONE', 'none', None, '']:
        ...
        branches[id_col] = [id_prefix + '_' + str(x) + '_' + id_suffix for x in range(len(branches))]
        logger.warning(f'id_col is not specified. Branch id columns are read/generated using default: {id_col}.')

    # check duplicated id
    _branches = branches.copy()
    _branches.reset_index(drop=True, inplace=True)
    _branches['count'] = _branches.groupby(id_col)[id_col].transform('cumcount').astype(int)
    for bi, b in _branches.iterrows():
        if b['count'] >= 1:
            _branches.rename(index={bi: b[id_col] + '-' + str(b['count'])}, inplace=True)
        else:
    ...
    logger.debug(f'Renaming {n} id_col duplicates. Convention: BRANCH_1, BRANCH_1 --> BRANCH_1, BRANCH_1-2.')

    # precision correction
    branches = reduce_gdf_precision(branches, rounding_precision = branches_ini['global']['rounding_precision'])  # recommned to be larger than e-8
    logger.debug(f"Reducing precision of the GeoDataFrame. Rounding precision (e-){branches_ini['global']['rounding_precision']} .")
    ...
    if branches_ini['global']['allow_intersection_snapping'] is True:
        ...
        branches = snap_branch_ends(branches, offset=snap_offset, max_points=2, logger=logger)
        logger.debug(
...
def _generate_nodes(branches: gpd.GeoDataFrame, logger = logging):
    ...
    nodes = pd.DataFrame([Point(l.coords[0]) for li, l in branches['geometry'].iteritems()] +
...
def setup_channels(branches: gpd.GeoDataFrame = None):
    ...
    start_points = gpd.GeoDataFrame([{_channels.index.name: idx, 'geometry': Point(line.coords[0])}
                                     for idx, line in _channels.geometry.iteritems()], crs=_channels.crs)
    start_points_ = snap_nodes_to_lines(_channels, start_points, offset, logger)
    end_points = gpd.GeoDataFrame([{_channels.index.name: idx, 'geometry': Point(line.coords[-1])}
                                   for idx, line in _channels.geometry.iteritems()], crs=_channels.crs)
    ...
    for channel_id in _channels.index:
        ...
        channel_line_coords[0] = (start_points_.loc[channel_id, 'geometry'].x,
                                  start_points_.loc[channel_id, 'geometry'].y)
        channel_line_coords[-1] = (end_points_.loc[channel_id, 'geometry'].x,
...
def setup_manholes(branches: gpd.GeoDataFrame = None,
    ...
    if branches is None:
        logger.info(f'No branch data is available meaning that no manholes will be created.')
        manholes_ = None
    else:
        ...
        if manholes_ini_fn is None and manholes_fn is None:
            ...
            manholes_ = read_gpd(manholes_fn, id_col, rename_map, required_columns, required_dtypes)
            ...
            if not generate_manholes:
                ...
                manholes_, branches_ = _generate_default_manholes_on_branches(branches, manholes_ini, id_col, id_prefix, id_suffix, logger)
        else:
            manholes_ini = parse_ini(manholes_ini_fn)
            if not generate_manholes:
                # use user manholes only
                ...
                logger.info(f'Manhole are read from {manholes_fn}.')
                manholes_user = read_gpd(manholes_fn, id_col, rename_map, required_columns, required_dtypes)
                ...
                manholes_ = _merge_user_manholes_to_generated_manholes(manholes_, manholes_user, snap_offset,
                                                                      logger)
...
def _generate_default_manholes_on_branches(branches: gpd.GeoDataFrame, manholes_ini: configparser.ConfigParser,
                                           id_col: str = "MANHOLE_ID", id_prefix:str = '', id_suffix:str = '', logger=logging):
    ...
    manholes_generated, branches_generated = __generate_manholes(branches, manholes_ini, id_col, id_prefix,
                                                                 id_suffix)
    # fill
    logger.info('Filling generated manhole based on defaults specified in manholes_ini_fn')
    fill_method = 'default'
    ...
    if manholes_ini['global']['bedlevel_source'] not in [None, 'default']:
        logger.info('bedlevel interpolated from raster source: ' + manholes_ini['global']['bedlevel_source'])
    if manholes_ini['global']['bedlevel_shift'] != 0.0:
        logger.info(
            'bedlevel shifted for: ' + str(manholes_ini['global']['bedlevel_shift']) + 'm')
        manholes_filled['bedlev'] = manholes_filled['bedlev'] + manholes_ini['global']['bedlevel_shift']
    if manholes_ini['global']['streetlevel_source'] not in [None, 'default']:
        logger.info(
            'streetlevel interpolated from raster source: ' + manholes_ini['global']['streetlevel_source'])
        temp = sample_raster_values(points=manholes_filled,
    ...
    if manholes_ini['global']['streetlevel_shift'] != 0.0:
        logger.info(
            'streetlevel shifted for: ' + str(manholes_ini['global']['streetlevel_shift']) + 'm')
        manholes_filled['streetlev'] = manholes_filled['streetlev'] + manholes_ini['global']['streetlevel_shift']
    return manholes_filled, branches_generated


def __generate_manholes(branches: gpd.GeoDataFrame, manholes_ini: configparser.ConfigParser,
                        id_col: str = 'MANHOLE_ID', id_prefix='MANHOLE_', id_suffix=''):
    ...
    nodes_pipes = __get_pipe_stats_for_manholes(nodes_pipes, 'where', 'ORIG_BRANCH_ID', ';'.join)
    ...
    nodes_channels = _nodes_channels.set_index(channels.index.name
                                               ).merge(pd.DataFrame(channels
                                                                    ).drop(columns=[channels.index.name, 'geometry']
    ...
    manholes_generated.loc[:, id_col] = ['%s_%s_%s' % (id_prefix, str(x), id_suffix) for x in
                                         range(len(manholes_generated))]
    manholes_generated.index = ['%s_%s_%s' % (id_prefix, str(x), id_suffix) for x in range(len(manholes_generated))]
    ...
    for pi, p in pipes.iterrows():
        ...
        try:
            pipes.at[pi, "MANHOLE_UP"] = manholes_dict[
                cs[0]]  # FIXME BMA: generalize or use fixed datamodel name in HydroMT
        except:  # no manholes at outlets
            pipes.at[pi, "MANHOLE_UP"] = ''
        try:
            pipes.at[pi, "MANHOLE_DN"] = manholes_dict[
                cs[-1]]  # FIXME BMA: generalize or use fixed datamodel name in HydroMT
        except:  # no manholes at outlets
...
def _merge_user_manholes_to_generated_manholes(manholes_generated: gpd.GeoDataFrame, manholes_user: gpd.GeoDataFrame,
                                               snap_offset: float = 0., logger=logging):
    """
    snap user manholes to generated manholes and overwrite generated manholes fields
    """
    manholes_merged = merge_nodes_with_nodes_prior_by_snapping(manholes_user, manholes_generated)
...
#                   roughness
# ===============================================================
def setup_roughness(branches: gpd.GeoDataFrame,
...
def _setup_rougnness_from_default(branches: gpd.GeoDataFrame,
                                  roughness_ini: dict, logger = logging):
...
def setup_crosssections(branches: gpd.GeoDataFrame,
    ...
    if generate_crosssections_from_branches is True and branches is not None:
        ...
        for c in crosssection_ini['global']['shift_col']:
            ...
            branches_[downstream_shift_col] = branches_[downstream_shift_col].combine_first(branches_[downstream_c_col])

        # setup crsdef from branches - upstream and downstream
        crsdefs = pd.concat([_setup_crsdefs_from_branches_at(branches_,
                                                             at='start',
                                                             ...
                                                             is_shared='True',
                                                             logger=logger),
                             _setup_crsdefs_from_branches_at(branches_,
                                                             at='end',
        ...
        crslocs = pd.concat([_setup_crslocs_from_branches_at(branches_,
                                                             at='start',
                                                             col_prefix=upstream_prefix,
                                                             col_suffix=upstream_suffix,
                                                             ...
                                                             crsdefid_col='definitionId',  # convention in Delft3D FM
                                                             logger=logger),
                             _setup_crslocs_from_branches_at(branches_,
                                                             at='end',
                                                             col_prefix=downstream_prefix,
                                                             col_suffix=downstream_suffix,
    ...
    if crosssections_fn is not None and crosssection_type == 'xyz':
        crosssections = read_gpd(crosssections_fn, id_col, rename_map, required_columns, required_dtypes, clip_geom = pilot.buffer(100))
        logger.info(
            'Crosssections are added from: ' + crosssections_fn)
        ...
        if use_infile_order is True:
            crosssections = crosssections.groupby(level=0).apply(geometry.xyzp2xyzl, (['ORDER']))
        else:
        ...
        logger.info(
            'Crosssections are snapped to nearest branch using ' + str(snap_method) + 'method.' )
        geometry.find_nearest_branch(branches=branches_, geometries=crosssections, method=snap_method) # FIXME: what if the line intersect with 2/wrong branches?
        ...
        if  drop_snap_failed == True:
            ...
            if len(_old_ids) != len(_new_ids):
                logging.warning(f'Crosssection with id: {list(set(_old_ids) - set(_new_ids)) } are dropped: unable to find closest branch. ')


        # setup crsdef from xyz
        crsdefs = pd.concat([crsdefs,
                             pd.DataFrame(
                                 {'id': crosssections.index.to_list(),
                                  ...
                                  'frictionid': branches_.loc[crosssections.branch_id.to_list(), 'frictionId'], # lower case key means temp keys (not written to file)
                                  })]
        ...
        crslocs = crslocs[~crslocs.branchid.isin(crosssections.branch_id.unique())] # FIXME change to branchId everywhere
        crslocs = pd.concat([crslocs,
                             pd.DataFrame({
                                 'id': [f'{bid}_{bc:.2f}' for bid, bc in zip(crosssections.branch_id.to_list(), crosssections.branch_offset.to_list())],
    ...
    for ri,r in crsdefs.iterrows():
        if r['type'] == 'rectangle':
            if np.all(r[["width", "height"]] > 0):
                ...
                logger.error(f"Rectangle crosssection definition of {r['id']} have 0 m width/height." +
        ...
        if r['type'] == 'trapezoid':
            if np.all(r[["t_width", "height"]] > 0):
                ...
                logger.error(f"Trapezoid crosssection definition of {r['id']} have 0 m t_width/height. " +
                         "Issue might have been caused due to incorrect values in branches data layer. Please revise. ")
        ...
        if r['type'] == 'circle':
            if np.all(r[["diameter"]] > 0):
                ...
                logger.error(f"Circle crosssection definition of {r['id']} have 0m diameter. " +
                             "Issue might have been caused due to incorrect values in branches data layer. Please revise. ")

    return crsdefs, crslocs, branches_


def _setup_crsdefs_from_branches_at(branches: gpd.GeoDataFrame,
                                    at: str = '',
                                    col_prefix: str = '',
                                    col_suffix: str = '',
                                    shape_col: str = 'SHAPE',
    ...
    logger.debug(f'Generating crsdefs using the following columns in GeoDataFrame: ' +
                 f'{shape_col}, {diameter_col}, {height_col}, {width_col}, {t_width_col}, {frictionid_col}')

    # set crossection definition id for branch
    branches = __set_crsdefid_for_branches(branches,
                                           shape_col=shape_col,
                                           diameter_col=diameter_col,
                                           height_col=height_col,
                                           width_col=width_col,
    ...
    crsdefs = __get_crsdef_from_branches(branches,
                                         shape_col=shape_col,
                                         diameter_col=diameter_col,
                                         height_col=height_col,
                                         width_col=width_col,
...
def _setup_crslocs_from_branches_at(branches: gpd.GeoDataFrame,
                                    at: str = 'start',
                                    col_prefix: str = '',
                                    col_suffix: str = '',
                                    shift_col: str = 'shift',
    ...
    logger.debug(f'Generating crslocs at {at} of the branch using the following columns in GeoDataFrame: ' +
                 f'{shift_col}, {crsdefid_col}')

    #  get crosssection locations
    crslocs = __get_crsloc_from_branches(branches,
                                         at=at,
                                         shift_col=shift_col,
                                         crsdefid_col=crsdefid_col)
    return crslocs
...
def __set_crsdefid_for_branches(branches: gpd.GeoDataFrame,
                                shape_col: str = 'SHAPE',
                                diameter_col: str = 'DIAMETER',
                                height_col: str = 'HEIGHT',
                                width_col: str = 'WIDTH',
    ...
    for bi in circle_indexes:
        branches.at[bi, crsdefid_col] = 'circ_d{:,.3f}_{:s}'.format(branches.loc[bi, diameter_col], crs_type)
    ...
    for bi in rectangle_indexes:
            branches.at[bi, crsdefid_col] = 'rect_h{:,.3f}_w{:,.3f}_c{:s}_{:s}'.format(
                branches.loc[bi, height_col], branches.loc[bi, width_col], branches.loc[bi, closed_col], crs_type)
    ...
    for bi in trapezoid_indexes:
        slope = (branches.loc[bi, t_width_col] - branches.loc[bi, width_col]) / 2 / branches.loc[bi, height_col]
        branches.at[bi, crsdefid_col] = 'trapz_s{:,.1f}_bw{:,.1f}_bw{:,.1f}_c{:s}_{:s}'.format(
            slope, branches.loc[bi, width_col], branches.loc[bi, t_width_col], branches.loc[bi, closed_col], crs_type)

    return branches


def __get_crsdef_from_branches(branches: gpd.GeoDataFrame,
                               shape_col: str = 'SHAPE',
                               diameter_col: str = 'DIAMETER',
                               height_col: str = 'HEIGHT',
                               width_col: str = 'WIDTH',
    ...
    for i in branches[crsdefid_col].unique():
        b = branches.loc[branches[crsdefid_col] == i,
                         [shape_col, diameter_col, height_col, width_col, t_width_col, closed_col, frictionid_col]
        ].drop_duplicates().to_dict('index')
        bi = list(b.keys())[0]
        crsdef.append({
            'id': i,
            'type': b[bi][shape_col],
...
def __get_crsloc_from_branches(branches: gpd.GeoDataFrame,
                               shift_col: str = 'shift',
                               crsdefid_col: str = 'definitionId',
                               at: str = 'start',
                               offset:float = 0.0):
...
def setup_bridges(branches: gpd.GeoDataFrame,
    ...
    if branches is not None:
        bridges = _setup_structures_defaults(branches,
                                             bridges_ini, bridges_fn, 'bridge',
                                             id_col, branch_query, snap_method, snap_offset, rename_map,
                                             required_columns, required_dtypes, pilot = pilot, logger = logger
                                             )
    ...
    if not any(x is None for x in [branches, bridges, crsdefs]):
        bridges, crsdefs = _setup_structures_crsdefs(branches=branches, structures=bridges, crsdefs=crsdefs, roughness_ini = roughness_ini,
                                                     crs_type='bridge', structures_ini=bridges_ini, logger=logger)

    logger.info(str(len(bridges)) + ' bridges are set up.')
    return bridges, crsdefs


def setup_gates(branches: gpd.GeoDataFrame,
                crsdefs: gpd.GeoDataFrame = None,
                crslocs: gpd.GeoDataFrame = None,
                roughness_ini_fn: str = None,
    ...
    # setup structure defaults
    if branches is not None:
        gates = _setup_structures_defaults(branches,
                                           gates_ini, gates_fn, 'gate',
                                           id_col, branch_query, snap_method, snap_offset, rename_map, required_columns,
                                           required_dtypes, pilot = pilot, logger = logger
                                           )
    ...
    # setup crosssections
    if not any(x is None for x in [branches, gates, crsdefs, crslocs]) and (gates_ini['global']['interpolate_crosssections'] == True):

        gates_crsdefs, gates_crslocs = _interpolate_branch_crosssections_at_gates(branches, gates, crsdefs, crslocs, logger)
...
def setup_pumps(branches: gpd.GeoDataFrame,
                crsdefs: gpd.GeoDataFrame = None,
                crslocs: gpd.GeoDataFrame = None,
                roughness_ini_fn: str = None,
    ...
    # setup structure defaults
    if branches is not None:
        pumps = _setup_structures_defaults(branches,
                                           pumps_ini, pumps_fn, 'pump',
                                           id_col, branch_query, snap_method, snap_offset, rename_map, required_columns,
                                           required_dtypes, pilot = pilot, logger = logger
                                           )
...
def setup_culverts(branches: gpd.GeoDataFrame,
                   crsdefs: gpd.GeoDataFrame = None,
                   crslocs: gpd.GeoDataFrame = None,
                   roughness_ini_fn: str = None,
    ...
    # setup structure defaults
    if branches is not None:
        culverts = _setup_structures_defaults(branches,
                                              culverts_ini, culverts_fn, 'culvert',
                                              id_col, branch_query, snap_method, snap_offset, rename_map,
                                              required_columns, required_dtypes, pilot = pilot, logger = logger
                                              )
    ...
    if not any(x is None for x in [branches, culverts, crsdefs]):
        culverts, crsdefs = _setup_structures_crsdefs(branches=branches, structures=culverts, crsdefs=crsdefs, roughness_ini = roughness_ini,
                                                      crs_type='culvert', structures_ini=culverts_ini, logger=logger)

    logger.info(str(len(culverts)) + ' culverts are set up.')
    return culverts, crsdefs


def setup_compounds(structures: list = [],
                    roughness_ini_fn: str = None,
    ...
    if len(structures) > 0:
        ...
        for ss in structures:
            if ss is not None:
                ...
                structures_.append(ss[['branch_id', 'branch_offset', 'branch_type', 'geometry']])
    ...
    if len(structures_) > 0:
        ...
        if compounds_ini_fn is not None:
            ...
            if generate_compounds is True:
                ...
                compounds_generated = _setup_generated_compounds(structures_, compounds_ini, id_col, id_prefix, id_suffix, snap_offset, logger)
            ...
            if compounds_fn is not None:
                ...
                compounds_user = _setup_user_compounds(structures_, compounds_fn, id_col, rename_map, required_columns,
                                                  required_dtypes, pilot, logger)
...
def _setup_generated_compounds(structures: pd.DataFrame, compounds_ini: configparser, id_col: str = 'STRUCT_ID',id_prefix:str = '', id_suffix:str = '',
                               snap_offset: float = 1, logger=logging):
    ...
    for bi in groups_by_branchid:
        ...
        groups_by_branchid_chainage = bi_group.groupby((bi_group["branch_offset"] / snap_offset).round(0)).groups
        for bi_c in groups_by_branchid_chainage:
            ...
            if np.any(bi_c_group["branch_type"] == 'Pipe'
                      ) and len(bi_c_group) > 1:
                logger.warning(f'Compound added: Pipe {compound["branchId"]} does not accept more than one structures. Please check: ' +
    ...
    # sort stuctureIds
    compounds_['structureIds'] = compounds_.structureIds.sort_values().apply(lambda x: sorted(x))
    compounds_["structureIds"] = compounds_["structureIds"].str.join(';')

    return compounds_


def _setup_user_compounds(structures: pd.DataFrame,
                          compounds_fn: str = None,
                          id_col: str = None,
                          rename_map: dict = None,
                          required_columns: list = None,
    ...
    # read user compounds
    compounds = read_gpd(compounds_fn, id_col, rename_map, required_columns, required_dtypes, clip_geom=pilot)
    ...
    for k, v in compounds.groupby(axis=1, level=0, dropna=True).groups.items():
        if len(v) > 1:
            compounds[k + 's'] = compounds[v].replace(
                {'None': None}).values.tolist()  # concat duplicated columns to list
            compounds[k + 's'] = compounds[k + 's'].apply(
                lambda c: list(set(x for x in c if x is not None)))  # remove None and duplicates

    # setup user compounds
    if 'structureIds' not in compounds.columns:
        logger.error('compounds must have structureIds. Please specify structureId in ini.')
    else:
        compounds_ = pd.DataFrame(columns=['branchId', 'chainage', 'type', 'structureIds', 'geometry'],
        ...
        for i, cmp in compounds[['branch_id', 'structureIds']].iterrows():
            ...
            if not set(cmp['structureIds']).issubset(set(structures.index.to_list())):
                logger.error('Compound not added: not all structures in compound have been setup successfully. Please check: ' +
            ...
            if len (structures.loc[cmp['structureIds'], 'branch_id'].unique()) > 1:
                logger.error('Compound not added: structures in the compounds are on not the same branch. Please check: ' +
                             f'{i} : {";".join(cmp["structureIds"])}')
                continue
            # check if all structures in a compound have the correct branch_id
            elif not np.all(structures.loc[cmp['structureIds'], 'branch_id'].unique() == cmp['branch_id']):
                logger.warning(f'Compound added: structures in the compounds are located on {structures.loc[cmp["structureIds"], "branch_id"].unique()[0]}, '+
            ...
            if np.any(structures.loc[cmp['structureIds'], 'branch_type'] == 'Pipe'
                      ) and len(cmp['structureIds']) > 1:
                logger.warning(f'Compound added: Pipe {cmp["branch_id"]} does not accept more than one structures. Please check: ' +
                               f'{i} : {";".join(cmp["structureIds"])}')
                pass

            # setup successful --> get chainage
            compounds_.at[i, 'branchId'] = structures.loc[cmp['structureIds'], 'branch_offset'].mean()
            compounds_.at[i, 'chainage'] = structures.loc[cmp['structureIds'], 'branch_offset'].mean()
            compounds_.at[i, 'type'] = 'compound'
            compounds_.at[i, 'structureIds'] = cmp['structureIds']
            compounds_.at[i, 'geometry'] = structures.loc[cmp['structureIds'], 'geometry'][0]  # estimate locations
...
def _compare_compounds(compounds_generated, compounds_user, logger = logging):
    ...
    combined_dfs = pd.concat([compounds_generated["structureIds"], compounds_user["structureIds"]])
    ...
    for cmp_i, cmp_n in combined_dfs_counts.items():
        if len(cmp_i.split(';'))>1 and cmp_n < 2:
            if cmp_i in compounds_generated["structureIds"].values:
                logger.warning(f"Compounds: generated compound structure of {cmp_i} could not be found in user specified compounds." +
...
def _setup_structures_defaults(branches: gpd.GeoDataFrame,
    ...
    if branch_query is not None:
        try:
            ...
            branches_ = None
            logger.error('branch_query specified for bridges is invalid.  Please check structure section in ini.')
    else:
        branches_ = branches
    if len(branches_) == 0:
        logger.warning('Structures are not added: there are no branches to add. Please check structure section in ini.')
    ...
    if structures_fn is None:
        ...
        try:
            structures = read_gpd(structures_fn, None, rename_map, required_columns, required_dtypes, clip_geom=pilot)
            logger.info(
                'Structures are added from: ' + structures_fn)
        except:
            structures = None
            logger.error('Structures filename specified is invalid . Please check structure section in ini.')
    ...
    if structures is not None:
        ...
        if id_col is not None:
            ...
            structures[id_col] = structures.index  # retain the id_col as columns for setting up the model
            logger.info(
                'Structures ids are identified from: ' + id_col)
        else:
            id_col = 'STRUC_ID'
            structures[id_col] = [structures_type + '_' + str(x) for x in range(len(structures))]
            logger.info(
                'Structures ids are identified from: ' + id_col)

        # setup branch_id - snap bridges to branch (inplace of bridges, will add branch_id and branch_offset columns)
        logger.info(
            'Structures are snapped to nearest branch within a snap_offset = ' + str(snap_offset))
        geometry.find_nearest_branch(branches=branches_, geometries=structures, method=snap_method, maxdist=snap_offset,
        ...
        if structures_ini['global']['drop_snap_failed'] == True:
            ...
            if len(_old_ids) != len(_new_ids):
                logging.warning('Structure with id:' + ','.join(
                    list(set(_old_ids) - set(_new_ids))) + ' are dropped: unable to find closes branch. ' + \
        ...
        if structures_ini['global']['compare_snap_with_column'] == True:
            if branches.index.name not in structures.columns:
                logging.error(
                    "Failed to execute: compare_snap_with_column = True. branch index could not be found in structures: " + branches.index.name)
            else:
                for i, struct in structures.iterrows():
                    if struct['branch_id'] != struct[branches.index.name]:
                        logger.warning(f"Structure {i} is snapped to : " + struct['branch_id'] +
                                       ", which does not match user specified branch: " + struct[branches.index.name] +
                                       ". Please check.")

        # setup successful - fill in values from default
        if structures_ini['global']['append_data_columns_based_on_ini_query'] == True:
            structures = append_data_columns_based_on_ini_query(structures, structures_ini)
            logger.info(
        ...
        for bi, b in structures.iterrows():
            if branches.loc[b['branch_id'], 'branchType'] == 'Pipe':
                ...
                logger.info(
                    'Structures are altering branchType from Pipe to SewerConnection at: ' + b['branch_id'])

    return structures


def _setup_structures_crsdefs(branches: gpd.GeoDataFrame,
    ...
    if structures_ini is not None:
        ...
        structures = pd.merge(_structures, branches.reset_index(drop=True), left_on='branch_id',
                              right_on=branches.index.name,
                              suffixes=('', '_b')).set_index(_structures.index.name)
        ...
        if structures_ini['global']['use_branch_friction'] == True:
            ...
            assert {'material_col'}.issubset(structures_ini['global'].keys()
                               ), "'material_col' must be specified in structure ini"
            # regenerate frictionId
            structures = _setup_rougnness_from_default(structures.drop(columns = ['frictionId', 'rough_type', 'rough_val']),
                                                       roughness_ini,
                                                       logger)
            ...
            assert {'frictype_col', 'fricval_col'
            ...
            for si, s in structures.iterrows():
                ...
                s[structures_ini['global']['frictype_col']], s[structures_ini['global']['fricval_col']])

        else:
            logger.error('No friction methods is chosen. Please check [global] section in ini.')

        # setup crosssection definition
        if {'shape_col', 'diameter_col', 'height_col', 'width_col', 't_width_col', 'closed_col'
            }.issubset(structures_ini['global'].keys()):
            structures_crsdefs = _setup_crsdefs_from_branches_at(structures,
                                                                 at='',
            ...
            logger.error("Failed to setup crosssection definition for structures. " +
        ...
        if structures_crsdefs is not None:
            crsdefs = pd.concat([crsdefs, structures_crsdefs]).drop_duplicates(subset = 'id').reset_index(drop=True)
            logger.info("Succeeded to setup crosssection definition for structures. " +
                        "Appending to exiting crosssection definitions")

    return structures, crsdefs


def _interpolate_branch_crosssections_at_gates(branches:gpd.GeoDataFrame, structures: gpd.GeoDataFrame, crsdefs, crslocs, logger = logging):
    ...
    structures = pd.merge(structures, branches.reset_index(drop=True), left_on='branch_id',
                          right_on=branches.index.name,
                          suffixes=('', '_b')).set_index(structures.index.name)
    structures[structures.index.name] = structures.index

    # check if the required columns are in the structures
    assert {'branch_id', 'branch_offset', 'branch_length', 'startdefinitionId',
         'enddefinitionId'}.issubset(structures.columns
               ), "Branches must have 'branch_id', 'branch_offset', 'branch_length', 'startdefinitionId', 'enddefinitionId' in the columns name."

    structures_crsdefs, structures_crslocs = __interpolate_crs_at_gates(structures, crsdefs, crslocs)
...
def __interpolate_crs_at_gates(structures, crsdefs, crslocs):
    ...
    if crslocs is not None:
        for bi, b in structures.iterrows():
            ...
            try:
                start_crsloc = crslocs.query(f'branchid == "{b.branch_id}" & chainage < {b.branch_offset}').sort_values('chainage').iloc[-1]
            except:
                start_crsloc = None
            try:
                end_crsloc = crslocs.query(f'branchid == "{b.branch_id}" & chainage > {b.branch_offset}').sort_values('chainage').iloc[0]
            except:
                end_crsloc = None
            ...
            if all(v is None for v in [start_crsloc, end_crsloc]):
                ...
                start_crsloc = end_crsloc = [v for v in [start_crsloc, end_crsloc] if v is not None][0]
            else:
                # both crs available
                pass
            ...
            structures_crslocs.loc[bi, 'shift'] = np.interp(b['branch_offset'], [start_crsloc['chainage'], end_crsloc['chainage']],
            ...
            if start_crsdef.type.isin(['xyz', 'yz']).any():
                structures_crsdefs.loc[bi, 'width'] = np.interp(b['branch_offset'], [0, b['branch_length']],
                                                                  [max(start_crsdef['xylength'].values[0]), max(end_crsdef['xylength'].values[0])])
                # add shift from profile to shift
                structures_crslocs.loc[bi, 'shift'] = structures_crslocs.loc[bi, 'shift'] + np.interp(b['branch_offset'], [0, b['branch_length']],
                                                                  [min(start_crsdef['zCoordinates'].values[0]), min(end_crsdef['zCoordinates'].values[0])])
                ...
                raise NotImplementedError('cross section type does not support interpolation. use rectangle, xyz and yz.')
...
def _disable_velocityheight_on_pipes(structures, logger = logging):
    ...
    assert ('branch_type' in structures.columns), "Structure must have 'branch_type' in columns"
    assert ('useVelocityHeight' in structures.columns), "Structure must have 'useVelocityHeight' in columns"
...
def setup_fixedweirs(fixedweirs_fn: str = None,
                     fixedweirs_fn_ini: str = None,
                     id_col: str = None,
                     split_offset:float = 100.,
                     rename_map: dict = None,
                     required_columns: list = None,
                     required_dtypes: list = None,
                     pilot: gpd.GeoDataFrame = None,
    ...
    if fixedweirs_fn is None:
        ...
        try:
            fixedweirs = read_gpd(fixedweirs_fn, None, rename_map, required_columns, required_dtypes, clip_geom=pilot)
            logger.info(
                'Fixedweirs are added from: ' + fixedweirs_fn)
        except:
            fixedweirs = None
            logger.error('Fixedweirs filename specified is invalid . Please check structure section in ini.')
    ...
    if fixedweirs is not None:
        ...
        if id_col is not None:
            ...
            fixedweirs[id_col] = fixedweirs.index  # retain the id_col as columns for setting up the model
            logger.info(
                'Fixedweirs ids are identified from: ' + id_col)
        else:
            id_col = 'STRUC_ID'
            fixedweirs[id_col] = ["fixedweir" + '_' + str(x) for x in range(len(fixedweirs))]
            logger.info(
                'Structures ids are identified from: ' + id_col)
...
#                   BOUNDARIES
# ===============================================================
def setup_boundaries(boundaries_fn: str = None,
    ...
    if boundaries_fn is None:
        ...
        try:
            boundaries = read_gpd(boundaries_fn, None, rename_map, required_columns, required_dtypes) # do not clip boundaries
            logger.info(
                'Boundaries are added from: ' + boundaries_fn)
        except:
            boundaries = None
    ...
    if boundaries is not None:
        ...
        if boundaries_fn_ini is not None:
            ...
            boundaries = append_data_columns_based_on_ini_query(boundaries, boundaries_ini)


        # setup boundaries id
        if id_col is not None:
            boundaries.index = boundaries[id_col]
            boundaries[id_col] = boundaries.index  # retain the id_col as columns for setting up the model
            logger.info(
                'Boundaries ids are identified from: ' + id_col)
        else:
            id_col = 'BC_ID'
            boundaries[id_col] = ["Boundaries" + '_' + str(x) for x in range(len(boundaries))]
            logger.info(
                'Boundaries ids are identified from: ' + id_col)
...
def setup_subcatchments(manholes: gpd.GeoDataFrame,
    ...
    **rename_map, required_columns, required_dtypes
    ...
    if manholes is None:
        ...
        logger.info(
            f'Subcatchments are directly read from subcatchments_fn.')

        # read
        # FIXME make clip buffer accessible from ini file
        subcatchments = read_gpd(subcatchments_fn, None, rename_map, required_columns, required_dtypes, clip_geom=pilot.buffer(100))
        subcatchments.index = subcatchments[id_col]

        # check
        if {'SUBCAT_ID', 'MANHOLE_ID', 'cl_slope', 'cl_flat',
       'cl_stretch', 'op_slope', 'op_flat', 'op_stretch', 'rf_slope',
       'rf_flat', 'rf_stretch', 'up_slope', 'up_flat', 'up_stretch',
       'inhabitant', 'company', 'dwf_def', 'meteo_id'}.issubset(subcatchments.columns
                                         ):
            needs_filling = False
        else:
            needs_filling = True
        ...
        if needs_filling is True:
            ...
            subcatchments = _fill_subcatchments(subcatchments, subcatchments_ini, landuse_fn,logger = logger)

    elif (subcatchments_fn is None and subcatchments_ini_fn is not None):
        logger.info(
            f'Subcatchments are generated from subcatchments_ini_fn.')
        ...
        subcatchments = _generate_subcatchments(manholes, subcatchments_ini, region_fn, barriers_fn, id_col=id_col,snap_offset = snap_offset, logger=logger)

        # fill subcatchment
        _subcatchments = subcatchments.copy()
        subcatchments = _fill_subcatchments(subcatchments, subcatchments_ini, landuse_fn = landuse_fn, logger=logger)

    # get xy coords of the subcatchments based on manholes
    if subcatchments is not None:
        _s_ = pd.merge(subcatchments.reset_index(drop=True), manholes.reset_index(drop=True),
                                 left_on=manholes.index.name, right_on=manholes.index.name,
                                 suffixes=('', '_m')).set_index(subcatchments.index.name)
...
def _generate_subcatchments(manholes: gpd.GeoDataFrame,
    ...
    if method == 'thiessen':
        subcatchments = __generate_thiessen_subcatchments(manholes, region_fn, barriers_fn,
        ...
        subcatchments = __generate_equal_subcatchments(manholes, region_fn, barriers_fn,
                                                    how = subcatchments_ini['global']['how'],
                                                    join_by = subcatchments_ini['global']['join_by'],
                                                    snap_offset = snap_offset,
                                                    id_col=id_col,
        ...
        raise NotImplementedError('method not recongnised. Please use one of the following: thiessen, equal')

    return subcatchments

def __generate_equal_subcatchments(manholes: gpd.GeoDataFrame,
                                                region_fn: str = None,
                                                barriers_fn: str = None,
                                                how:str = 'join',
    ...
    # manhole overlay
    if how == 'intersection':
        raise ValueError("method equal could only be used with how ==  join. Please specify how and join_by")
    elif how == 'join':
        ...
        if manhole_join_by not in _manholes.columns:
            raise NotImplementedError(f'Join_by {join_by} is not accepted. Only accepts branch index column name.')
        else:
            manholes_ = _manholes.assign(region_index=  _manholes[manhole_join_by].str.split(";"))
            manholes_ = gpd.GeoDataFrame(pd.DataFrame(manholes_).explode('region_index', ignore_index = True))
            manholes_.index = manholes_[_manholes.index.name]
    else:
        raise NotImplementedError(f'how: {how} is not accepted. Only accepts intersection/join.')
    ...
    for region_index, subregion in region.iterrows():
        ...
        if not isinstance(subregion, gpd.GeoDataFrame):
            ...
            subregion = gpd.GeoDataFrame(subregion.to_frame().T, geometry = 'geometry', crs = region.crs)
            subregion = subregion.buffer(0) # applying a zero distance buffer can fix the invalid geometries
            subregion = gpd.GeoDataFrame(subregion.to_frame().rename(columns={0:'geometry'}), geometry='geometry', crs=region.crs)

        submanholes = gpd.GeoDataFrame(manholes_.loc[manholes_["region_index"] == region_index,: ], geometry = 'geometry', crs = _manholes.crs).drop_duplicates()
        ...
        if len(submanholes) > 0:
            ...
            _subcatchments = gpd.GeoDataFrame(geometry=[p.buffer(
                np.sqrt(subregion.area/len(submanholes)/np.pi), resolution=16
            ) for p in submanholes.geometry.to_list()], index=submanholes.index)
    ...
    logger.info(f"Subcatchments are linked to manholes using manhole ids as {manholes.index.name}. ")
    ...
    logger.info(f"Subcatchments are equally generated within region using ids as {id_col}. ")

    # check area
    if region.area.sum() * 99.8/100 > subcatchments_.area.sum() : # allowed error due to precision
        logging.warning('Total subcatchments area lower than the specified region area. Please check intermediate exports. ')

    return subcatchments_

def __generate_thiessen_subcatchments(manholes: gpd.GeoDataFrame,
                                                region_fn: str = None,
                                                barriers_fn: str = None,
    ...
    for region_index, subregion in region.iterrows():
        ...
        if len(submanholes) > 0:
            ...
            if len(submanholes) <= 1:
                ...
                _subcatchments = gpd.GeoDataFrame(geometry = subregion.geometry.to_list(), index = submanholes.index)
            else:
                # apply thiessen polygon for more than 2 points
                try:
                    _subcatchments = get_thiessen_polygons(submanholes, subregion, barriers = None, logger = logger)
                    _subcatchments = gpd.GeoDataFrame(
                        {id_col: ['subcat_%s' % (i) for i in range(len(_subcatchments))],
                         'geometry': _subcatchments.geometry})
                except:
                    logger.warn(f'Subcatchment {subregion.index.to_list()} could not be used thus skipped. ')
                    continue
    ...
    if how == 'intersection':
        subcatchments_ = gpd.sjoin(subcatchments, manholes_, how="inner", op='intersects')
        ...
        if len(leftover_subcatchments) > 0:
            ...
            leftover_subcatchments_ = __link_subcatchments(leftover_subcatchments_, manholes_, barrier, snap_offset, logger = logger)
            leftover_subcatchments_[id_col] = [f'subcat_{i}' for i in range(len(subcatchments_), len(subcatchments_) + len(leftover_subcatchments_))]
            leftover_subcatchments_[manholes.index.name] = leftover_subcatchments_["TO_INDEX"]
            subcatchments_ = pd.concat([subcatchments_, leftover_subcatchments_]).drop_duplicates(keep=False)
            logger.info("Isolated subcatchments are linked to manholes circumventing barriers. ")
...
def _fill_subcatchments(subcatchments: gpd.GeoDataFrame, subcatchments_ini: configparser.ConfigParser, landuse_fn:str = None, logger = logging):
    ...
    subcatchments = __fill_subcatchments_with_defaults(subcatchments, subcatchments_ini, logger=logger)
    ...
    # overwrite defaults with landuse
        ...
        subcatchments = __fill_subcatchments_with_landuse(subcatchments,landuse_fn,logger=logger)
...
def __fill_subcatchments_with_defaults(subcatchments: gpd.GeoDataFrame, subcatchments_ini: configparser.ConfigParser, logger = logging):
    """fill subcatchments attributes using fill method (overwrite exisitng columns)"""


    # fill in defaults
    subcatchments = append_data_columns_based_on_ini_query(subcatchments, subcatchments_ini)
    ...
    for landuse_col in ['cl', 'op', 'rf', 'up']:
        ...
        for runoff_col in ['flat', 'stretch', 'slope']:
            subcatchments[f"{landuse_col}_{runoff_col}"] = landuse_area * subcatchments[f"{runoff_col}_ratio"]

    return subcatchments

def __fill_subcatchments_with_landuse(subcatchments: gpd.GeoDataFrame,
                                     landuse_fn: str,
                                     logger = logging):
    ...
    if landuse_fn is None:
        ...
        logger.error(f"Can not setup subcatchment from landuse raster. no landuse_fn is specified.")
        pass

    else:

        logger.info(f"Filling in subcatchment based on {landuse_fn}... might take a minute")
        landuse_df = None

        try:

            # read raster
            landuse_da = read_raster(raster_fn = landuse_fn, id_col = 'landuse', nodata = -999)
            logger.debug("read landuse raster (nodata is recongnised as -999)")

            # parse raster to nwrw classes
            logger.debug('parsing landuse classes (1. Closed paved; 2. Open paved; 3. Roof;4. Unpaved)' )
            ...
            landuse_df = landuse_da.raster.zonal_stats(subcatchments, stats="sum").to_pandas()
            landuse_df = landuse_df.multiply(np.prod(landuse_da.raster.res))
            landuse_df = landuse_df.rename(columns = {'cl_sum':'cl', 'op_sum':'op', 'rf_sum': 'rf',  'up_sum':'up'})
        ...
        if landuse_df is not None:
            ...
            for lu in ['cl', 'op', 'rf', 'up']:
                for rf in ['flat', 'stretch', 'slope']:
                    subcatchments[f'{lu}_{rf}'] = landuse_df[lu] * subcatchments[f'{rf}_ratio']
...
def __link_subcatchments(subcatchments: gpd.GeoDataFrame, manholes: gpd.GeoDataFrame, barriers: gpd.GeoDataFrame = None,
                         maxdist: float = float('inf'), max_area: float = float('inf'),
                         ignore_disconnected: bool = True, logger=logging):
    ...
    if maxdist is None:
        ...
        maxdist = 10
        logger.debug('Max distance less than 10 meters is not recommended. Will be overruled by 10 m.')
    ...
    subcatchments['FROM_INDEX'] = subcatchments['FROM_INDEX'].astype(type(links['FROM_INDEX'][0]))
    subcatchments['TO_INDEX'] = subcatchments['TO_INDEX'].astype(type(links['TO_INDEX'][0]))
...
def setup_precipitation(precipitation_fn:str = None,
                        region_fn:str = None,
                        id_col: str = None,
                        rename_map: dict = None,
                        required_columns: list = None,
    ...
    if not forced_upon == 'drr':
        raise NotImplementedError("Can not setup precipitation. Only support forced_upon = drr")
    else:
        logger.info("setting up precipitation for drr")

    # setup default global precipitation
    if subcatchments is None:
        logger.error("Can not setup precipitation. Subcatchments do not exist for setting up precipitation")
    else:
        x1,y1,x2,y2 = subcatchments.unary_union.bounds
        precipitation_region = gpd.GeoDataFrame({id_col:['global'], 'geometry':[box(x1,y1,x2,y2)]})

    # read forcing regions
    if region_fn is not None:
        precipitation_region_ = read_gpd(region_fn, id_col, rename_map, required_columns, required_dtypes)
        if precipitation_region_.geom_type[0] == 'Polygon':
            logging.info(f"precipitation is applied within polygon regions specified in: {region_fn}")
        elif precipitation_region_.geom_type[0] == 'Point':
            logging.info(f"precipitation is applied at station specified in: {region_fn}")
            precipitation_region_ = get_thiessen_polygons(precipitation_region_, precipitation_region, barriers=None, logger=logger)
    else:
        logging.info(f"precipitation is applied globally within the entire subcatchments domain")
    ...
    n = len(G) - len(G.drop_duplicates().index)
    precipitation_region_ = _precipitation_region_[_precipitation_region_.index.isin(G.drop_duplicates().index)]
    logger.debug(f'Removing {n} sub-regions which have duplicated geometry.')
    ...
    if len(subcatchments_wo_meteo) > 0:
        logger.warning(f"{len(subcatchments_wo_meteo)} subcatchments without meteo_id identified. These subcatchments will not be used."+
    ...
    if len(precipitation.columns) == 1 and 'global' in precipitation.columns[0].lower():
        ...
        precipitation_ = pd.concat([precipitation] * len(subcatchments_), axis=1, ignore_index=True)
...
def setup_dflowfm(
    ...
    if model_type == '1d' or model_type == '1d2d':
        ...
        dfmmodel.network.set_branches(
            dm.__getattribute__('branches'), id_col="id"
        )
        ...
        if 'bridges' in vars(dm).keys():
            dfmmodel.structures.io.bridges_from_datamodel(dm.__getattribute__('bridges'))
        ...
        if 'culverts' in vars(dm).keys():
            dfmmodel.structures.io.culverts_from_datamodel(dm.__getattribute__('culverts'))

        if 'compounds' in vars(dm).keys():
            dfmmodel.structures.io.compounds_from_datamodel(dm.__getattribute__('compounds'))
        ...
        dfmmodel.network.generate_1dnetwork(
            ...
            seperate_structures=network1d_ini['global']['seperate_structures'], #FIXME: will seperate structures nonetheless, because the argument is not checked
            urban_branches=dm.__getattribute__('branches').query('branchType == 2').index.to_list())
        ...
        if 'manholes' in vars(dm).keys():
            ...
            dfmmodel.storage_nodes.add_manholes(
                dm.__getattribute__('manholes')
            )
    ...
    if model_type == '2d' or model_type == '1d2d':
        ...
        if grid_fn is None and grid_2d_ini_fn is None:
            raise ValueError(f"Model type {model_type} is specified but no 2D grid is generated or specified.")

        elif grid_fn is not None and grid_2d_ini_fn is not None:
            raise ValueError(f"Both grid generation option is selected and an 2D grid ({grid_fn}) is specified. Choose one of the options.")
            ...
            raise ValueError(f"Use of an existing 2D grid is not yet implemented in the code")
            ...
            if grid_2d_ini['global']['generate_grid'] == 'polygon':
                ...
                if grid_polygon_fn is None:
                    raise ValueError(f"Grid generation based on polygon is selected but no polygon is provided "
                                     f"(grid_polygon_fn)")
                ...
                if grid_2d_ini['global']['shape_cell'] == 'square':
                    ...
                    logger.info(f"An {grid_2d_ini['global']['shape_cell']} grid with resolution of {grid_2d_ini['global']['cellsize']} within a polygon is created")

                elif grid_2d_ini['global']['shape_cell'] == 'triangle':
                    # TODO: implement grid generation with triangle cells
                    raise ValueError(f"Use of triangular cells is not yet implemented in the code")
                else:
                    raise ValueError(f"{grid_2d_ini['default']['shape_cell']} is not an existing option for the shape of "
            ...
            # generate grid via bounding box
                ...
                raise ValueError(f"Bounding box method  is not yet implemented in the code")
            else:
                raise ValueError(f"An unknwon grid generation method is specified. Please select grid generation based "
        ...
        if 'bedlevel' not in grid_2d_ini['default']:
            raise ValueError(f"No default bed level is specified in {grid_2d_ini_fn} under [default]. "
        ...
        if grid_2d_ini['default']['bedlevel'] is None:
            logger.info(f"No default bed level is specified, so it will be set to -999.0")
            grid_2d_ini['default']['bedlevel'] = -999.0


        if bedlevel_fn is not None and grid_fn is not None:
            raise ValueError(f"The model generator cannot yet handle the adding of 2D grids with Z-values (bed levels)")
            ...
            if bedlevel_fn.endswith('.pol'):
                raise ValueError(f"Setting of 2D bedlevels via polygons is not yet implemented ")
            else:
                ...
                logger.info(f"Bed level file ({bedlevel_fn}) is interpolated on the grid using "
                            f"{grid_2d_ini['global']['bedlevel_averagingType']} averagingType")
        else:
            logger.info(f"No bedlevel file is specified, so default bed level of {grid_2d_ini['default']['bedlevel']}"
                        f" m AD will be uniformly applied by setting it in the MDU-file")

        # Set default bed level in MDU-file
        logger.info(f"Uniform bed level of {grid_2d_ini['default']['bedlevel']} m AD is set in the MDU-file.")
        ...
        if 'roughness' not in grid_2d_ini['default']:
            raise ValueError(f"No default roughness value is specified in {grid_2d_ini_fn} under [default]. "
        ...
        if 'roughness_type' not in grid_2d_ini['global']:
            raise ValueError(f"No roughness_type is specified for 2D. Please specify roughness_type in {grid_2d_ini_fn} under [global]. "
        ...
        if grid_2d_ini['default']['roughness'] is None:
            logger.info(f"No default roughness is specified, so it will be set to -999.0")
        ...
        if roughness_fn is not None:
            raise ValueError(f"Use of roughness layer is not implemented in the code. Only an uniform roughness can be applied")
        ...
        if isinstance(grid_2d_ini['global']['roughness_type'],str):
            roughness_type = _rewrite_roughness_type(grid_2d_ini['global']['roughness_type'])
            logger.info(f"The roughness type is rewritten from {grid_2d_ini['global']['roughness_type']} to {roughness_type}.")
            ...
            raise ValueError(f"Specified roughness_type in {grid_2d_ini_fn} under [global] is incorrect."
                             f"Choose between Chezy, Manning, WallLawNikuradse or WhiteColebrook.")


        # Set default bed level in MDU-file
        logger.info(f"Uniform roughness of {grid_2d_ini['default']['roughness']}  is set in the MDU-file.")
        dfmmodel.mdu_parameters['UniFrictCoef'] = grid_2d_ini['default']['roughness']
        logger.info(f"Uniform roughness of {grid_2d_ini['global']['roughness_type']}  is set in the MDU-file.")
    ...
    if model_type == '1d2d':
        ...
        dfmmodel.network.links1d2d.generate_1d_to_2d(max_distance=50,
                                                     branchid=dm.__getattribute__('branches').query('branchType == 2').index.to_list())
...
def setup_drr(dm, dfmmodel, rr_concept:str = 'nwrw', logger = logging):
    ...
    if 'precipitation' in vars(dm).keys():
        drrmodel.external_forcings.io.precip_from_datamodel(dm.__getattribute__('precipitation'))
    ...
    if rr_concept == 'nwrw':
        ...
        if 'subcatchments' in vars(dm).keys():
            drrmodel.nwrw.io.nwrw_from_datamodel(catchments=dm.__getattribute__('subcatchments'))
    ...
    # TODO
    drrmodel.d3b_parameters = {'TimeSettings':
                                   {'TimestepSize':60,
...
def write_drrmodel(drrmodel, output_dir, name:str = 'DRR', logger = logging):
    ...
    rr_writer = DFlowRRWriter(drrmodel, output_dir=output_dir,
                              name=name)
    rr_writer.copyRRFiles()
    rr_writer.update_config()
...
def setup_validation(branches:gpd.GeoDataFrame,
    ...
    if validate_1dnetwork:
        validate_1dnetwork_connectivity(branches, plotit=plotit, exportpath=exportpath, logger=logger)
        validate_1dnetwork_flowpath(branches, branchType_col = 'branchType', plotit=plotit, exportpath= exportpath, logger=logger)
    return None
...
def validate_branch_flowpath(
    branches: gpd.GeoDataFrame, ax=None
):
    ...
    ctx.add_basemap(source='http://mt0.google.com/vt/lyrs=s&hl=en&x={x}&y={y}&z={z}',alpha = 0.5,  crs=branches.crs.to_epsg(), ax=ax)
    ...
    for outlet in network_outlets:
        c = random_color()
        outlet_G = G.subgraph(list(dict(nx.bfs_predecessors(RG, outlet)).keys()) + [outlet])
        nx.draw_networkx(
            outlet_G,
            pos,
            node_size=50,
