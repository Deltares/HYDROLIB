...
def find_critical_intersections(
    branches: gpd.GeoDataFrame, offset: float = 0.01, logger=logging
):
    """
    ...
    points_index = [li for li, l in branches.geometry.iteritems()] + [
        li for li, l in branches.geometry.iteritems()
    ]
...
def intersect_branches(
    branches: gpd.GeoDataFrame, offset: float, id_col="BRANCH_ID", logger=logging
):
    """function to make branch intersect within a given threshold (offset)"""
    endpoints = [
        {"idx": idx, "geometry": Point(line.coords[0])}
        for idx, line in branches.geometry.iteritems()
    ]
    endpoints = endpoints + [
        {"idx": idx, "geometry": Point(line.coords[-1])}
        for idx, line in branches.geometry.iteritems()
    ]
...
def split_branches(
    ...
    if spacing_col is None:
        ...
        split_branches = _split_branches_by_spacing_const(
            branches, spacing_const, id_col=id_col, logger=logging
        )
        ...
        for spacing_subset, branches_subset in branches.groupby(spacing_col):
            if spacing_subset:
                split_branches_subset = _split_branches_by_spacing_const(
                    branches_subset, spacing_subset, id_col=id_col, logger=logging
                )
            else:
...
def _split_branches_by_spacing_const(
    branches: gpd.GeoDataFrame, spacing_const: float, id_col="BRANCH_ID", logger=logging
):
    """
    ...
    for bid, b in s.iterrows():
        new_edges = [
            LineString([Point(p1), Point(p2)])
            for p1, p2 in zip(b.geometry.coords[:-1], b.geometry.coords[1:])
        ]
        lengths = np.r_[
            0,
            np.cumsum(
                np.hypot(
                    np.diff(b.geometry.coords, axis=0)[:, 0],
                    np.diff(b.geometry.coords, axis=0)[:, 1],
                )
            ),
        ]
        ...
        edge_invertup.extend(
            np.interp(lengths[:-1], [0, lengths[-1]], [b.INVLEV_UP, b.INVLEV_DN])
        )
        edge_invertdn.extend(
            np.interp(lengths[1:], [0, lengths[-1]], [b.INVLEV_UP, b.INVLEV_DN])
        )
        edge_bedlevup.extend(
            np.interp(lengths[:-1], [0, lengths[-1]], [b.BEDLEV_UP, b.BEDLEV_DN])
        )
...
def merge_nodes_with_nodes_prior_by_snapping(
    nodes: gpd.GeoDataFrame,
    nodes_prior: gpd.GeoDataFrame,
    offset: float = 1,
    logger=logging,
...
# FIXME BMA: check if HydroMT has point snapping function with offset
def snap_nodes_to_lines(
    lines: gpd.GeoDataFrame, nodes: gpd.GeoDataFrame, offset: float = 1, logger=logging
):
    """
    ...
    for line_id, line in lines.geometry.iteritems():
        ...
        if len(points) > 0:
            snapped += len(points)
            nodes_.at[points.index, "geometry"] = points.apply(
...
def leftjoin_gpd_by_index(
    data1: gpd.GeoDataFrame, data2: gpd.GeoDataFrame, case_sensitive=False
):
...
def in_box(towers, bounding_box):
    return np.logical_and(
        np.logical_and(
            bounding_box[0] <= towers[:, 0], towers[:, 0] <= bounding_box[2]
        ),
        np.logical_and(
            bounding_box[1] <= towers[:, 1], towers[:, 1] <= bounding_box[3]
        ),
    )
...
def get_thiessen_polygons(
    ...
    if region is None:
        ...
        vor_in_region = gpd.clip(
            vor_in_bounds, region
        ).explode()  # explode multi polygons
        vor_in_region = vor_in_region[
            vor_in_region.geom_type.values == "Polygon"
        ]  # get rid of points
    ...
    if barriers is None:
        ...
        vor_in_region_avoid_barriers = gpd.overlay(
            vor_in_region, barriers, how="symmetric_difference"
        ).explode()
        ...
        logging.warn(
            f"barriers type is not recognized. Please use Polygon or LineString."
        )
        vor_in_region_avoid_barriers = vor_in_region
...
def link_polygons_to_points(
    ...
    from_points = [
        ((poly.centroid.x, poly.centroid.y), poly.Index)
    ...
    # # group branch ends based on off set
    dist = distance.cdist(
        np.array([p for p, pi in from_points]), np.array([p for p, pi in to_points])
    )
        ...
        while not found and len(to_points) > 0:
            ...
            if mindist <= maxdist:
                ...
                if barriers is not None:
                    ...
                    intersect = np.any(
                        [b.intersects(c_to_m) for _, b in barriers.geometry.iteritems()]
                    )
                    if intersect:
                        found = False
...
def redistribute_vertices(geom, distance):
    ...
    if geom.geom_type == "LineString":
        parts = [
            LineString([p1, p2]) for p1, p2 in zip(geom.coords[:-1], geom.coords[1:])
        ]
        parts_new = []
        for part in parts:
