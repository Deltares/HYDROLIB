...
__all__ = ["generate_crosssections"] #, "process_crosssections", "validate_crosssections"]
...
def generate_crosssections(
    ...
    for c in crosssections_ini['global']['shift_col']:
        ...
        crosssections[upstream_shift_col] = crosssections[upstream_shift_col].combine_first(
            crosssections[upstream_c_col].rename({}))
        crosssections[downstream_shift_col] = crosssections[downstream_shift_col].combine_first(crosssections[downstream_c_col])

    # setup crsdef from branches - upstream and downstream
    crsdefs = pd.concat([_setup_crsdefs_from_branches_at(crosssections,
                                                         at='start',
                                                         ...
                                                         is_shared='True',
                                                         logger=logger),
                         _setup_crsdefs_from_branches_at(crosssections,
                                                         at='end',
    ...
    crslocs = pd.concat([_setup_crslocs_from_branches_at(crosssections,
                                                         at='start',
                                                         col_prefix=upstream_prefix,
                                                         col_suffix=upstream_suffix,
                                                         ...
                                                         crsdefid_col='definitionId',  # convention in Delft3D FM
                                                         logger=logger),
                         _setup_crslocs_from_branches_at(crosssections,
                                                         at='end',
                                                         col_prefix=downstream_prefix,
                                                         col_suffix=downstream_suffix,
...
def setup_crosssections(branches: gpd.GeoDataFrame,
    ...
    if crosssections_fn is not None and crosssection_type == 'xyz':
        crosssections = read_gpd(crosssections_fn, id_col, rename_map=rename_map, required_columns=required_columns, required_dtypes=required_dtypes,
                                 required_query=required_query,
                                 clip_geom=pilot.buffer(clip_buffer), clip_predicate=clip_predicate,
                                 logger = logger)
        ...
        if len(crosssections) > 0:
            ...
            crosssections = read_gpd(crosssections_fn, id_col, rename_map=rename_map, required_columns=required_columns,required_dtypes=required_dtypes,
                                     required_query=required_query,
                                     clip_geom=pilot.buffer(clip_buffer + 1000), clip_predicate=clip_predicate,
                                     logger=logger)
            ...
            if  drop_snap_failed == True:
                ...
                if len(_old_ids) != len(_new_ids):
                    logger.warning(f'Crosssection with id: {list(set(_old_ids) - set(_new_ids)) } are dropped: unable to find closest branch. ')
