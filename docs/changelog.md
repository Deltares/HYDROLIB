...
| --- | --- | --- | --- |
| n/a | n/a | n/a | n/a |
| <center> **Updates and maintenance** <center> | | | |
...
| New functionality | Description | Adjustments to workflow? | Link to D-HyDAMO Wiki |
| --- | --- | --- | --- |
| More robust implementation of complex controllers | Checks for duplicates and presence of structures. | New compulsory element in reading complex controllers (id_limit_complex_controllers) | [complex controllers](https://hkvconfluence.atlassian.net/wiki/spaces/DHYD/pages/2077949955/Complexe+sturing) |
| Improvements for modelling sub-areas | ‘Clip’-function extended with possibility for choosing feature selection or clipping;<br> - possibility to read time series from result of another model;<br> - possibility of Q-H relation as boundary. | Optional : Cliptype =’select’  of  cliptype =”clip” in ‘read_gpkg_layer ’ (see  [example](https://github.com/Deltares/HYDROLIB/blob/main/hydrolib/notebooks/Hydrolib-D-Hydamo_usage_introduction_v050.ipynb) ) | [sub-area models](https://hkvconfluence.atlassian.net/wiki/spaces/DHYD/pages/2987786242/Genereren+van+deelmodellen) |
| Logging improved | All messages are collected in one report. | Logging needs to be initialised in notebook (see [example](https://github.com/Deltares/HYDROLIB/blob/main/hydrolib/notebooks/Hydrolib-D-Hydamo_usage_introduction_v050.ipynb) ) | n/a |
| <center> **Updates en maintenance** <center> | | | |
| Implement Python 3.12,  HYDROLIB-core  0.10 andn  MeshKernel  8.2.2. | General update of dependencies.  Models that were generated with D-HyDAMO <0.5.0 are not compatible with DHYDRO  2026.01. | n/a | n/a |
| Benchmark-tests  implemented | In the background, test models are run and validated after updates. | n/a | n/a |
| Warning for crosssections intersecting multiple hydroobjects | When multiple cross-sections intersect a hydroobject, a random one is used. This is not desired behaviour. | Specify a snapping method | [snapping methods](https://hkvconfluence.atlassian.net/wiki/spaces/DHYD/pages/3042902020/Koppelmethodes+voor+geometrie+n+en+waterlooptakken) |
| Renaming folders to import model output in GUI | (Output)folders  and the MDU-file need to have specific names to enable import of model output in the GUI. | - rename‘ fm ’ folder  to ‘ dflowfm’<br>  - change model name ( mdu ) to ‘ DFM’<br> - provide output folder as attribute to dimr.add_crs() (see  [example](https://github.com/Deltares/HYDROLIB/blob/main/hydrolib/notebooks/Hydrolib-D-Hydamo_usage_introduction_v050.ipynb)) | [known issues](https://hkvconfluence.atlassian.net/wiki/spaces/DHYD/pages/2078245015/Bekende+problemen) |
| DHYDRO -output specification | In run.bat, it is now possible to specify a debuglevel and output file location (optional). | Add optional arguments to dimr.write_runbat ():   “ debuglevel ”  en ” runlog ”. | n/a |
| Keywords   MDU  adjusted | Unnecessary keywords were removed and HYDROLIB-core defaults are now used. | Different  keywords  specificied in workflow (see  [example](https://github.com/Deltares/HYDROLIB/blob/main/hydrolib/notebooks/Hydrolib-D-Hydamo_usage_introduction_v050.ipynb) ) | n/a |
| <center> **Bug fixes** <center> | | | |
| Removing  1D2D -links | Removal of 1D2D -links within polygons did not always work well. | n/a | n/a |
| Controllers for weirs with multiple openings | For openings with identical indices, a controller was only added to the last one. We removed the index-dependency. | n/a | n/a |
| Observation locations in RTC | Duplicate references to the same observation location are removed. | n/a | n/a |
| Error for crosssections with missing z-coordinates | For missing cross-section z-coordinates, the y-coordinate was used. Now an error is issued. | n/a | n/a |
